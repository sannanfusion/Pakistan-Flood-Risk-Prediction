import { RISK_COLORS, RISK_LABELS, ProvinceData } from '@/lib/types';
...
// PDF Generator
async function generatePDF(provinces: ProvinceData[], provinceId?: string) {
  try {
    ...
    filtered.forEach((province) => {
      ...
      doc.text(`Population: ${(province.population / 1e6).toFixed(1)}M | Deaths: ${province.deaths} | Houses Dmg: ${province.housesDamaged}`, 14, y + 28);
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center text-muted-foreground text-sm py-20">
          No province data available. Check your backend connection.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {provinces.map((province, i) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground font-mono">Live Data · {province.riskLevel.toUpperCase()}</p>
                  </div>
                </div>
                <span
              ...
              {/* Download button */}
              <button
                onClick={() => generatePDF(provinces, province.id)}
