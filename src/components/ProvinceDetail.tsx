...
export function ProvinceDetail({ province }: ProvinceDetailProps) {
  ...
  const riskBgClass =
    province.riskLevel === 'critical' ? 'bg-risk-critical/10 border-risk-critical/25' :
    province.riskLevel === 'high' ? 'bg-risk-high/10 border-risk-high/25' :
    province.riskLevel === 'medium' ? 'bg-risk-medium/10 border-risk-medium/25' : 'bg-risk-low/10 border-risk-low/25';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <MetricCard icon={<Droplets className="w-4 h-4 text-accent" />} label="30-Day Rain" value={`${province.rainfall30Day}mm`} />
        <MetricCard
...
function MetricCard({ icon, label, value, alert }: { icon: React.ReactNode; label: string; value: string; alert?: boolean }) {
  return (
    <div className={`p-2.5 rounded-xl bg-secondary/30 border transition-colors ${alert ? 'border-risk-high/30' : 'border-transparent'}`}>
